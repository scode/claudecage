...
Your project directory is mounted read-write so the agent can modify code. `~/.claude` and `~/.codex` are mounted
read-write so auth, sessions, and settings persist across runs. A few optional helper paths may also be mounted
(`~/.leiter`, `~/.gitconfig`, and resolved symlink targets from the agent state directories). A GitHub token can
...
claudecage codex -- "fix the build"  # pass a prompt to codex
...
- **`~/.claude`** — mounted read-write. Auth tokens, session state, history, and settings persist across ephemeral
  ...
  under `$HOME`.
- **`~/.codex`** — mounted read-write. Codex auth state, settings, history, rules, plugins, skills, worktrees, and
  caches persist across ephemeral container runs. Created automatically if it doesn't exist. If `~/.codex` is a symlink,
  its resolved path must be under `$HOME`.
- **`~/.leiter`** — mounted read-write if it exists. Not created automatically.
- **Symlink targets from `~/.claude` and `~/.codex`** — symlinks anywhere within either agent state directory are
  recursively resolved and their targets mounted read-only. This covers both top-level symlinks and nested ones. The
  traversal descends into real subdirectories but does not follow symlinks to directories, preventing cycles.
...
- **Filesystem**: only the project directory, `~/.claude`, and `~/.codex` (all read-write) are mounted, plus a few
  optional helper paths. The agent cannot see or access anything else on the host. If a GitHub token is configured (see
  Quickstart), it is injected as an environment variable.
...
- **Ephemeral**: each invocation is a fresh container (`--rm`). No state leaks between runs except through mounted host
  paths such as `~/.claude`, `~/.codex`, the project directory, and the optional helper mounts.
...
Symlink targets from `~/.claude` and `~/.codex` are validated to be under `$HOME`. Because both directories are
writable, a process inside the container could create symlinks pointing to other directories under `$HOME`, which would
become visible read-only on the next run. See [docs/security.md](docs/security.md) for the full threat model and known
risk vectors.
