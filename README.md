...
- `claudecage image build` — builds a Docker image (Ubuntu 24.04 + Node 22 + system `bubblewrap` + claude-code +
  codex + Homebrew + `gh` + `uv` + `ghstack`) with a non-root user matching the host user's uid/gid. Only needs to be
  run once.
