...
fn codex_responds_to_prompt() {
    ...
    let output_file = tempfile::NamedTempFile::new().expect("failed to create output temp file");
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_claudecage"))
        .args([
            ...
            "-o",
            &output_path,
            "respond with exactly the word PING",
        ])
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(stderr)
    ...
    let output = fs::read_to_string(output_file.path()).expect("failed to read codex output file");
