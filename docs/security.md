...
authenticate inside the container via `/login`, Claude Code writes an OAuth bearer token to `~/.claude/.credentials.json`.
Because `~/.claude` is a bind mount, this file lands on the host — and because the mount is read-write, every later
session that mounts Claude state can read it. Anyone with this token can make API calls as you.
